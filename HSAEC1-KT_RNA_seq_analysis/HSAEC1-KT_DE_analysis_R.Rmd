---
title: "HSAEC1-KT_RNA_seq_R_analysis"
...
```{R warning=FALSE}
combined_counts <- read_delim("//joffrey-smb.igs.umaryland.edu/projects-t3/EFUNG/riley.risteen/Cb_HSAEC1-KT_inf/counts/master_count_table.counts", 
                              delim = "\t", escape_double = FALSE, 
                              col_names = FALSE, trim_ws = TRUE)

metadata <- read_delim("//joffrey-smb.igs.umaryland.edu/home/riley.risteen/Cb_hsaec_inf_rnaseq/hsaec1-kt_cb_inf_sample_metadata.csv", 
                       delim = ",", escape_double = FALSE, 
...
str_spec_metadata$metadata_Cb175 <- metadata %>% filter(strain =="Cb175")
...
str_spec_metadata$metadata_Cb182 <- metadata %>% filter(strain =="Cb182")
...
filt_metadata$Cb175_3h <- metadata %>% filter(strain =="Cb175" & time_point == "3h")
filt_metadata$Cb175_6h <- metadata %>% filter(strain =="Cb175" & time_point == "6h")
...
#create metadata files for 3h and 6h samples
filt_metadata$Cb182_3h <- metadata %>% filter(strain =="Cb182" & time_point == "3h")
filt_metadata$Cb182_6h <- metadata %>% filter(strain =="Cb182" & time_point == "6h")
...
```{r, echo=FALSE}
write.table(DE_data, file = "C:\\Users\\Riley\\Documents\\University of Maryland\\Bruno Lab\\Host_RNA_seq\\FINAL_DATA_AND_CODE\\HSAEC1-KT_all_Cb_inf_DE_Genes.csv", sep = ",", col.names = TRUE, row.names = F, quote = FALSE)
```

#Export count tables (raw and tpm)
merge all of the raw counts and export the table
```{r}
rawcountslist <- list(Cb175_3h_matrix, Cb175_6h_matrix, Cb182_3h_matrix, Cb182_6h_matrix, Cb190_3h_matrix, Cb190_6h_matrix)
...
write.table(rawcounts, file = "C:\\Users\\Riley\\Documents\\University of Maryland\\Bruno Lab\\Host_RNA_seq\\FINAL_DATA_AND_CODE\\HSAEC1-KT_raw_counts.csv", sep = ",", col.names = TRUE, row.names = F, quote = FALSE)
...
write.table(tpm_counts, file = "C:\\Users\\Riley\\Documents\\University of Maryland\\Bruno Lab\\Host_RNA_seq\\FINAL_DATA_AND_CODE\\HSAEC1-KT_tpm_counts.csv", sep = ",", col.names = TRUE, row.names = F, quote = FALSE)
```
